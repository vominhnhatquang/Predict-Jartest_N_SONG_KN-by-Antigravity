...
async function loadPerformanceData() {
    try {
        ...
        if (result.success && result.data) {
            ...
            const updateElement = (id, value, isPercent = false) => {
                const el = document.getElementById(id);
                if (el && value !== undefined && value !== null) {
                    const formattedValue = typeof value === 'number' ? value.toFixed(2) : value;
                    el.textContent = formattedValue + (isPercent ? '%' : '');
                }
            };
            ...
            updateElement('metric-mape', metrics.test_mape, true);
            updateElement('metric-accuracy', metrics.test_accuracy, true);
            updateElement('metric-model', metrics.model_type || metrics.best_model);
...
function displayResult(data) {
    ...
    const predictionValue = data.prediction !== undefined ? data.prediction : data.prediction_value;

    if (predictionValue !== undefined) {
        html += `<div class="prediction-value">${predictionValue.toFixed(2)}</div>`;
    }
